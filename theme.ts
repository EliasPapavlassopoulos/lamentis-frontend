...
export const THEMES = [PRIMARY] satisfies EthleteTheme[];

export type Theme = (typeof THEMES)[number]['name'];

export const createTailwindCssVar = (name: string) =>
    `rgb(var(--${name}) / <alpha-value>)`;
export const createTailwindRgbVar = (val: string | undefined) =>
    val ? `rgb(${val} / <alpha-value>)` : undefined;

export const createTailwindColorThemes = (
    themes: EthleteTheme[],
    prefix = 'et'
) => {
    ...
    for (const theme of themes) {
        ...
        twThemes[key] = {
            ...
            focus:
                createTailwindRgbVar(theme.primary.color.focus) ||
                createTailwindRgbVar(theme.primary.color.hover)!,
        ...
        twThemes[keyOn] = {
            ...
            focus:
                ...
                createTailwindRgbVar(theme.primary.onColor.default)!,
            active:
                createTailwindRgbVar(theme.primary.onColor.active) ||
                createTailwindRgbVar(theme.primary.onColor.default)!,
            disabled:
                createTailwindRgbVar(theme.primary.onColor.disabled) ||
                createTailwindRgbVar(theme.primary.onColor.default)!,
        };
    }
